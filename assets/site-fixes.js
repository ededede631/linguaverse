(function () {
  ...
  function ctx() {
    ...
    // 只在章节页和学习模块页更新 saved context
    // 课程列表页和首页不更新，避免页面上其他课程卡片的标题误判
    if (isChapterRoute || isLearnRoute) {
  ...
  function patchHome() {
    ...
    document.querySelectorAll("a,button").forEach(el => {
      ...
      // 检测级别
      let level = "初级";
      if (/高级|精通|N1|商务/.test(text)) level = "高级";
      else if (/中级|进阶|N2|N3/.test(text)) level = "中级";
      el.dataset.lvLangEntry = lang;
      el.dataset.lvLevelEntry = level;
      el.addEventListener("click", e => {
        if (!/课程|英语|日语|韩语|English|日本|韩国|初级|中级|高级/.test(text)) return;
        e.preventDefault();
        localStorage.setItem(FILTER_KEY, lang);
        // 设置上下文，避免跳转到错误级别
        const currentCtx = {};
        try { Object.assign(currentCtx, JSON.parse(localStorage.getItem(CONTEXT_KEY) || "{}")); } catch (_) {}
        currentCtx.language = lang;
        currentCtx.level = level;
        currentCtx.courseTitle = `${lang}${level}课程`;
        localStorage.setItem(CONTEXT_KEY, JSON.stringify(currentCtx));
        location.hash = "#/courses";
        toast(`已切换到${lang}${level}课程。`);
      }, true);
    });
  }
  ...
  function patchCourseFilter() {
    ...
    if (!document.getElementById("lv-filter-banner")) {
      ...
      h?.insertAdjacentHTML("afterend", `<div id="lv-filter-banner" class="lv-panel">${lang ? `当前筛选：${lang}课程 <button class="lv-ghost" data-lv-clear-filter="1">查看全部</button>` : "全部课程 <span style=\"color:#64748b;font-size:13px;margin-left:8px\">点击课程卡片进入学习</span>"}</div>`);
    }
    // 给所有课程卡片绑定点击事件：点击时根据卡片内容设置正确的语言和级别
    document.querySelectorAll("article,a,.rounded-2xl,.rounded-3xl").forEach(card => {
      ...
      let lv = "初级";
      if (/高级|精通|N1|商务/.test(t)) lv = "高级";
      else if (/中级|进阶|N2|N3/.test(t)) lv = "中级";
      if (lang && l !== lang) {
        card.style.display = "none";
      } else if (l && card.tagName === "A" && !card.dataset.lvCourseBound) {
        card.dataset.lvCourseBound = "1";
        // 在捕获阶段设置上下文，确保页面跳转前上下文已更新
        card.addEventListener("click", e => {
          const currentCtx = {};
          try { Object.assign(currentCtx, JSON.parse(localStorage.getItem(CONTEXT_KEY) || "{}")); } catch (_) {}
          currentCtx.language = l;
          currentCtx.level = lv;
          currentCtx.courseTitle = `${l}${lv}课程`;
          localStorage.setItem(CONTEXT_KEY, JSON.stringify(currentCtx));
        }, true);
      }
    });
